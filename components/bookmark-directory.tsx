...
export function BookmarkDirectory() {
  const { bookmarks: document } = useContent();
  const bookmarks = document.items;
  const categories = document.categories.map((item) => item.name);
  ...
  return <div className="directory-page">
    ...
    <div className="directory-layout">
      ...
      <section className="directory-results" aria-label="书签目录">
        ...
        {categories.map((name) => { const items = visible.filter((item) => item.category === name); return items.length > 0 && <section className="bookmark-section" key={name} aria-label={name}><h3><span>{name}</span><small>{filtered.filter((item) => item.category === name).length.toString().padStart(2, '0')}</small><i /></h3><div className="bookmark-cards">{items.map((item) => <a className="bookmark-card" href={item.url} target="_blank" rel="noopener noreferrer" key={item.id} aria-label={`${item.name}（在新标签页打开）`}><div className="bookmark-card-top"><span className="bookmark-monogram" aria-hidden="true">{item.name.slice(0, 2).toUpperCase()}</span><div><h4>{item.name}</h4><small>{new URL(item.url).hostname.replace(/^www\./, '')}</small></div><ArrowUpRight size={17} /></div><p>{item.description}</p><div className="directory-tags">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></a>)}</div></section>; })}
