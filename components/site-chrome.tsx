...
export function SiteHeader() {
  ...
  return <header className="site-header"><Link className="brand" href="/"><span className="brand-mark">A</span><span>开发阿雷</span></Link><nav>{links.map(([name, href]) => <Link className={path === href ? 'active' : ''} href={href} key={href}>{name}</Link>)}<Sheet><SheetTrigger className="nav-drawer-trigger"><span>+</span> 网站</SheetTrigger><SheetContent className="life-drawer"><SheetTitle>网站索引</SheetTitle><div>{[['书签', '/bookmarks'], ['友链', '/friends']].map(([name, href], index) => <Link href={href} key={href}><span>0{index + 1}</span><b>{name}</b><i>↗</i></Link>)}</div></SheetContent></Sheet><Sheet><SheetTrigger className="nav-drawer-trigger"><span>+</span> 生活</SheetTrigger><SheetContent className="life-drawer"><SheetTitle>生活索引</SheetTitle><div>{life.map(([name, href], index) => <Link href={href} key={href}><span>0{index + 1}</span><b>{name}</b><i>↗</i></Link>)}</div></SheetContent></Sheet></nav><DropdownMenu><DropdownMenuTrigger className="theme-toggle"><span className="theme-icon">{fresh ? '☼' : '☾'}</span>{fresh ? '清风' : '明月'}</DropdownMenuTrigger><DropdownMenuContent align="end" className="theme-menu"><DropdownMenuRadioGroup value={fresh ? 'fresh' : 'default'} onValueChange={setTheme}><DropdownMenuRadioItem value="fresh"><span className="theme-option-icon">☼</span>清风</DropdownMenuRadioItem><DropdownMenuRadioItem value="default"><span className="theme-option-icon">☾</span>明月</DropdownMenuRadioItem></DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu></header>;
}
