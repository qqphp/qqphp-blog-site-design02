...
export function ResearchHub({ type }: { type: 'ai' | 'investing' }) {
  ...
  return <div className={`research-hub research-${type}`}><header className="research-heading"><div><p>{page.label}</p><h1>{page.title}</h1></div><span>{page.description}</span></header>
    <div className="research-overview"><section className="research-focus"><p className="research-eyebrow">{type === 'ai' ? '从任务到方法' : pageSettings.investing.eyebrow}</p><h2>{type === 'ai' ? '先让一个小任务，变得更顺手。' : pageSettings.investing.title}</h2><div className="research-steps">{(type === 'ai' ? ['明确问题', '准备输入', '检查输出', '记录边界'] : pageSettings.investing.steps).map((step, i) => <span key={step}><small>0{i + 1}</small>{step}</span>)}</div><a href="#research-library"><CmsText page="投资页" name="01 浏览研究目录" /><ArrowDown size={15} /></a></section><aside className="research-index-card"><BookOpen size={22} /><strong>{String(total).padStart(2, '0')}<small><CmsText page="投资页" name="02 篇示例笔记" /></small></strong><p>{type === 'ai' ? '工作流、工具、提示词与学习路线，按主题收纳。' : pageSettings.investing.description}</p><span>{type === 'ai' ? '编辑示例 · 非实测工具榜单' : pageSettings.investing.note}</span></aside></div>
    ...
    </section><p className="research-disclosure">{type === 'ai' ? '本页为内容与实践框架示例，未接入实时产品资讯或实际评测结果。' : pageSettings.investing.disclosure}</p>
  </div>;
}
